...
public class KnobsController {
    ...
    private CursorRemoteControlsPage deviceRemotes;
    ...
    public KnobsController(ControllerHost host) {
        ...
        deviceRemotes = host.createCursorTrack(0, 0).createCursorDevice().createCursorRemoteControlsPage(8);
        for (int i = 0; i < 8; i++) {
            final RelativeHardwareKnob knob = surface.createRelativeHardwareKnob("CC_DEV_RMT_" + (i + 1));
            ...
            final RemoteControl param = deviceRemotes.getParameter(i);
            param.addBinding(knob);
            knob.setStepSize(1);
            knob.setSensitivity(0.02);
        }
        trackRemotes = host.createCursorTrack(0, 0).createCursorRemoteControlsPage("trackRemotes", 8, null);
        for (int i = 0; i < 8; i++) {
